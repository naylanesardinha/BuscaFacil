const USUARIO_VALIDO = {
    ...
    btnMostrar.addEventListener('click', () => {
        ...
        iconeOlho.className = visivel ? 'fa-solid fa-eye' : 'fa-solid fa-eye-slash';
        btnMostrar.title = visivel ? 'Mostrar senha' : 'Ocultar senha';
    });
    ...
    function setLoading(on) {
        ...
        btnLabel.textContent = on ? 'Verificando…' : 'Entrar';
...
function BuscarVeiculo() {
    ...
    if (carregando || !veiculosData) {
        ...
        messageIcon.className = 'fa-solid fa-spinner fa-spin';
        messageIcon.style.color = '#06b6d4';
        messageText.textContent = 'Carregando dados, tente novamente em instantes.';
        return;
    }
    ...
    const veiculoEncontrado = veiculosData.find(carro => {
        const chave = `${carro.nome.toLowerCase()} ${carro.ano}`;
        return termos.every(termo => chave.includes(termo));
    });
    ...
    if (veiculoEncontrado) {
        ...
        messageCard.classList.remove('hidden');
        messageIcon.className  = 'fa-solid fa-triangle-exclamation';
        messageIcon.style.color = '#ef4444';
        messageText.textContent = `Nenhum veículo encontrado para "${searchInput.value.trim()}".`;
    }
}
